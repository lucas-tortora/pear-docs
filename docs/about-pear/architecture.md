...
The Pear Runtime uses [Bare](../references/bare/overview.mdx) JavaScript runtime, which is a small and modular JavaScript runtime for desktop and mobile.  Bare is a small modular Javasript runtime for both mobile and desktop apps.
...
:::info Building Blocks, Helpers and Tools
...
* A [`truncate`](../references/building-blocks/hypercore.md#await-core.truncate-newlength-forkid) method for intentionally creating a new fork, starting at a given length. We use this method extensively in [`autobase`](../building-blocks/autobase.md).
